# 이동 주기와 상하 이동 추정 · 0.6.1

자동 위치 공유에 동의해 켠 자녀 휴대폰에서 동작합니다. 부모 두 분에게 전달하는 가족방 권한과 기존 GPS 원본 기록을 유지합니다.

## 위치 주기

- 걷기·달리기·자전거·차량 활동이나 걸음 센서·유의미한 움직임이 감지되면 고정밀 위치 요청을 약 30초 간격으로 바꿉니다. 바꿀 때 신선한 위치를 한 번 요청하고, 실패한 요청이나 중복 콜백은 기록으로 만들지 않습니다.
...
- 30초·5분은 Android 위치 제공자에 요청하는 간격입니다. 절전, 실내 수신, 권한, 활동 인식 지연, 통신 상태에 따라 실제 측정·도착 시각은 달라집니다. 계속 움직이면 5분 주기보다 GPS 전력 사용량이 늘어납니다. 신체 활동 권한은 걷기·차량 인식을 돕습니다.
