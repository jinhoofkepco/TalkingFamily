...
0.6.2는 특정 가족에게 전달되지 않을 때의 오류 구분·봇 재확인을 보완하고, 발송 전 수신 대기와 알림 후 화면 표시 지연을 줄입니다. [변경 사항과 확인 범위](docs/releases/0.6.2.md)
...
- **자동 위치 공유:** 자녀가 안내를 읽고 별도로 동의해 켠 경우에만 이동을 감지하면 약 30초마다, 움직임 근거가 없으면 유예 후 약 5분마다 새 위치를 요청합니다. GPS 흔들림만으로 빠른 주기를 켜지 않습니다. 필요한 위치·신체 활동·알림 권한을 요청하고 공유 중 알림을 표시합니다. 켜 둔 설정은 앱 재실행 때 이어가며, 자녀가 설정이나 알림에서 끄면 멈춥니다. 자동 위치 공유 설정에서 최근 자동 측정 시각과 보호자 기기의 수신 여부를 확인할 수 있습니다. 연결이나 메시지 수신만으로는 위치를 자동 수집하지 않습니다.
- **오르내림:** 걸음 센서·기압계·움직임으로 올라가기·내려가기의 시작과 종료를 추정하며, 지도 안의 시간 막대에서 상대 높이와 근거를 확인합니다. 정확한 층수나 계단·엘리베이터 여부를 판별하는 기능은 아닙니다. [30초 주기·상하 이동 구현 근거와 한계](docs/motion-and-height.md)를 확인하세요.
...
30초·5분은 위치 제공자에 요청한 간격이며 Android가 정확한 측정·전달 시각을 보장하지 않습니다. 위치 기능이 꺼져 있거나 위치 신호·절전·통신 상태가 좋지 않으면 기록이나 전송이 늦어질 수 있습니다. GPS 정확도와 기압 기반 높이 추정은 실내 환경·날씨·휴대 방식에 영향을 받습니다.
