...
0.6.4는 대화창 위의 자동 안내·오류 배너를 없애고 휴대폰 내부 진단 로그로 남깁니다. 대화 화면을 보고 있을 때는 새 메시지 알림을 띄우지 않으며, 밖에서는 기본 무음 알림 하나를 갱신합니다. **설정 → 메시지 알림**에서 소리를 켤 수 있습니다. 0.6.3의 적응형 메시지 수신과 이동 중 약 20초·정지 중 약 5분 위치 요청 주기는 유지합니다. Firebase나 별도 가족 서버를 사용하지 않습니다. [변경 사항과 확인 범위](docs/releases/0.6.4.md)
...
앱 밖의 새 알림은 상대방의 대화 메시지만 표시합니다. 대화 화면을 보고 있으면 메시지 알림을 지우고 추가 알림을 띄우지 않습니다. 다른 화면이나 앱 밖에서는 알림 하나를 갱신하며 기본 소리·진동·팝업은 꺼져 있습니다. **설정 → 메시지 알림 → 알림 소리**를 켜면 새 알림이 처음 나타날 때만 소리를 내며, 같은 알림이 갱신될 때는 다시 울리지 않습니다. Android에서 직접 바꾼 채널 설정과 알림 차단은 우선 적용됩니다. 위치·칭찬 알림은 보내지 않으며 Android가 요구하는 수신·위치 공유·별 아이콘 실행 알림은 무음으로 표시합니다.
...
- **자동 위치 공유:** 자녀가 안내를 읽고 별도로 동의해 켠 경우에만 이동을 감지하면 약 20초마다, 움직임 근거가 없으면 유예 후 약 5분마다 새 위치를 요청합니다. GPS 흔들림만으로 빠른 주기를 켜지 않습니다. 필요한 위치·신체 활동·알림 권한을 요청하고 공유 중 알림을 표시합니다. 켜 둔 설정은 앱 재실행 때 이어가며, 자녀가 설정이나 알림에서 끄면 멈춥니다. 자동 위치 공유 설정에서 최근 자동 측정 시각과 보호자 기기의 수신 여부를 확인할 수 있습니다. 연결이나 메시지 수신만으로는 위치를 자동 수집하지 않습니다.
- **오르내림:** 걸음 센서·기압계·움직임으로 올라가기·내려가기의 시작과 종료를 추정하며, 지도 안의 시간 막대에서 상대 높이와 근거를 확인합니다. 정확한 층수나 계단·엘리베이터 여부를 판별하는 기능은 아닙니다. [이동 주기·상하 이동 구현 근거와 한계](docs/motion-and-height.md)를 확인하세요.
- **GPS 흔들림 보정:** 자동 공유 중 Android 활동 인식으로 정지·걷기·차량 이동 등을 확인합니다. 정지 증거와 연속된 위치가 일치할 때 지도 표시 좌표를 고정해 작은 GPS 흔들림을 줄이고, 지도 안에 해당 기록 시점의 정지 추정 시간을 표시합니다. 실제 이동·오래된 활동 정보·측정 중단을 정지로 간주하지 않으며, 원본 GPS 값과 실제 측정 시각은 유지합니다. 신체 활동 권한이 없으면 기본 위치 공유를 계속하고 자동 위치 공유 설정에서 권한을 허용할 수 있습니다. 보정 표시는 두 휴대폰을 0.4.4 이상으로 업데이트한 뒤 새로 받은 자동 기록에 적용됩니다.
...
20초·5분은 위치 제공자에 요청한 간격이며 Android가 정확한 측정·전달 시각을 보장하지 않습니다. 받는 기기가 대화 화면을 닫아 둔 경우 새 기록 수신에도 최대 60초의 확인 주기가 적용될 수 있습니다. 위치 기능이 꺼져 있거나 위치 신호·절전·통신 상태가 좋지 않으면 기록이나 전송이 더 늦어질 수 있습니다. GPS 정확도와 기압 기반 높이 추정은 실내 환경·날씨·휴대 방식에 영향을 받습니다.
