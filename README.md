...
보호자의 **자녀 위치** 탭에서 OpenStreetMap 지도를 바로 확인합니다. 별도 지도 API 키 없이 지도 이동·확대·축소와 **최근 위치로** 이동을 사용할 수 있습니다. 마지막으로 받은 위치, 측정 시간, 위치 오차를 함께 표시하며 10분 이상 지난 위치에는 안내를 붙입니다. 위치를 받은 적이 없으면 빈 지도 대신 위치 공유 안내를 보여 줍니다. 지도 위의 자동 공유 상태 카드와 앱 상단의 일반 동작 안내 배너는 표시하지 않습니다. 입력 오류·전송 실패 안내는 확인할 수 있습니다.
