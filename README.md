...
0.6.5는 화면을 껐다가 잠금을 풀 때 별 아이콘이 돌아오지 않는 문제를 수정합니다. 화면 켜짐·잠금 해제 직후 표시 조건을 짧게 다시 확인해, 잠금 상태 변경이 늦게 반영돼도 별을 복구합니다. 기본 무음·단일 메시지 알림, 적응형 메시지 수신과 이동 중 약 20초·정지 중 약 5분 위치 요청 주기는 유지합니다. Firebase나 별도 가족 서버를 사용하지 않습니다. [변경 사항과 확인 범위](docs/releases/0.6.5.md)
